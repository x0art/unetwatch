  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <div className="flex items-center gap-2 border-b border-border bg-muted/30 px-4 py-3">
        ...
        <h3 className="font-mono text-xs font-bold uppercase tracking-widest">Risk Summary</h3>
        <span className="ml-auto hidden truncate font-mono text-[11px] text-muted-foreground sm:inline">
          {host.hostname || host.primaryIp} · {risk.totalRequests.toLocaleString()} req · {risk.riskRequests.toLocaleString()} risks
        </span>
      </div>
