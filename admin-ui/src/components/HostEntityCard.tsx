...
/** Host summary card — host = IP + hostname only (ADR 0001: no MAC/dept/user
 * identity). Risk is reframed: risks are ALLOW pattern-matches (need action);
 * enforcements (DENY) are handled by the proxy and shown separately. */
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <div className="flex items-center gap-2 border-b border-border bg-muted/30 px-4 py-3">
        ...
        <h3 className="font-mono text-xs font-bold uppercase tracking-widest">Host &amp; Risk Summary</h3>
        <span className="ml-auto hidden font-mono text-[11px] text-muted-foreground tabular-nums sm:inline">
          {risk.totalRequests.toLocaleString()} req · {risk.riskRequests.toLocaleString()} risks
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 divide-y divide-border md:grid-cols-2 md:divide-x md:divide-y-0">
        ...
        <div className="p-4 sm:p-5">
          <p className="mono-label mb-3">HOST</p>
        ...
        <div className="p-4 sm:p-5">
          <p className="mono-label mb-3">RISK PROFILE</p>
